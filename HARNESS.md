...
- Task 안에서 Red/Green을 나눌 때도 "컴파일이 실패하는지"/"테스트가 통과하는지"를 Task 단위로 미리
  확인하지 않는다 (3장 참고). 계획 파일의 모든 Task를 커밋까지 마친 뒤, 계획 단위로 한 번만 전체를
  확인한다.
...
1. **Red 커밋**: 아직 구현이 없어 컴파일이 실패하거나 assert가 실패하는, 그 기능의 핵심 동작만 검증하는
   ...
   실제로 컴파일이 실패하는지는 빌드해서 확인하지 않는다** — 이 확인도 2장에서 말하는 "계획 단위 빌드
   확인" 시점에 한꺼번에 이루어진다.
2. **Green 커밋**: 그 테스트를 통과시키는 최소 구현을 작성하고 커밋한다. 통과 여부도 Task 단위로 확인하지
   않고 계획 단위 빌드 확인 시점에 몰아서 확인한다.
