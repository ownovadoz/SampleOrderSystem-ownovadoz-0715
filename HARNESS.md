...
- 특히 파일 저장/조회(영속성)는 항상 `DataPersistence`의 `JsonValue`/`FileRepository<T>` 패턴을 써야 한다
  (파이프 구분 텍스트, CSV 등 자체 포맷 금지). 테스트 데이터 생성 기능(담당자가 앱을 수동으로 써보기 위한
  대량 더미 데이터 주입)은 `DummyDataGenerator` 패턴을 재사용해야 한다 — 단, gtest 단위 테스트 안에서 쓰는
  손으로 작성한 리터럴 픽스처(`Sample{"S-001", ...}` 등)는 이 규칙의 대상이 아니다.
