...
> **핵심 요약**: 공통 규칙은 `PRD.md`, 모듈별 상세 규칙은 `docs/requirements/*.md`, 개발 프로세스(하네스)는
> `HARNESS.md`. 모듈을 개발할 때는 `PRD.md` + 해당 모듈 문서 + `HARNESS.md` 세 개를 읽으면 된다.
...
- **`docs/requirements/*.md`** (모듈별 1개 파일): 해당 모듈에만 해당하는 상세 요구사항. 다른 모듈과 겹치는
  ...
  있습니다.
- **`HARNESS.md`**: *무엇을* 만들지가 아니라 *어떻게* 개발할지(서브에이전트 위임 방식, Task별 빌드 확인,
  TDD 커밋 순서, 네이밍 컨벤션, PoC 활용 방법)를 다룹니다. `docs/superpowers/plans/*.md`의 계획을 실제로
  구현할 때는 이 문서의 프로세스를 따릅니다. superpowers 플러그인 스킬(`subagent-driven-development` 등)은
  사용하지 않습니다 — 계획 파일은 순수 마크다운 Task 목록이므로 별도 스킬 없이 `Agent` 툴로 직접 위임해
  진행합니다.
